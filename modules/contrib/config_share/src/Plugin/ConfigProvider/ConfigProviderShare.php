...
class ConfigProviderShare extends ConfigProviderBase {
  ...
  public function addInstallableConfig(InMemoryStorage $installable_config, array $extensions = []) {
    // @todo: review whether we should be supporting non-default configuration
    // collections.
    $shared_config_to_add = $this->getSharedConfigToCreate($installable_config->listAll(), $extensions);
  ...
  protected function getSharedConfigToCreate(array $config_to_create, array $extensions = []) {
    ...
    if (!empty($config_to_create)) {
      ...
      $shared_storage = $this->getExtensionInstallStorage(static::ID);
